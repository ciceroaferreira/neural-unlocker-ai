...
export interface AudioRecordingState {
  ...
  startCapture: () => Promise<{
    onAudioProcess: (callback: (base64: string) => void) => void;
    stop: () => void;
  }>;
  stopCapture: () => void;
  togglePause: () => void;
...
export function useAudioRecording(): AudioRecordingState {
  ...
  const startCapture = useCallback(async () => {
    ...
    scriptNode.onaudioprocess = (e) => {
      ...
      // Accumulate raw buffer for WAV export
      rawBuffers.current.push(new Float32Array(data));

      // Convert and send to callback
      const pcm16 = float32ToInt16(data);
      const base64 = encode(new Uint8Array(pcm16.buffer));
      if (audioCallback) audioCallback(base64);
    };
    ...
    return {
      onAudioProcess: (callback: (base64: string) => void) => {
